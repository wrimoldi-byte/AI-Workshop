const $=s=>document.querySelector(s); const log=$('#log'); let stopped=false, best='', history=[];
async function status(){const s=await fetch('/api/status').then(r=>r.json()); $('#mode').textContent=s.mode==='gemini'?`● IA real · ${s.model}`:'● modo demo · sin API key'} status();
